...
要望申請フォームは、活動写真（最大3枚・1枚5MBまで）とPDF資料5種（1点10MBまで）を
同時に送信できる仕様のため、最大構成では1リクエストが約65MBになり得る。
...
| 5 | 添付資料（活動写真3枚必須・PDF 5種・確認チェック）。編集時（トークンアクセス時）は添付が任意になり、未添付の場合は既存ファイルを保持する |
