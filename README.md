...
**活動写真の規定枚数**：活動写真は「1枚以上」ではなく、規定枚数ちょうどが必須（要望申請3枚・完了報告2枚。`Validator::SUBMISSION_PHOTO_COUNT` / `Validator::REPORT_PHOTO_COUNT`、フォーム側は `PhotoSlots` の `maxSlots`）。この規定枚数チェックは新規申請・新規提出（`POST`）時のみ強制し、`validateSubmissionFiles()` / `validateReportFiles()`（`PUT` 時のマージ後の最終状態チェック）では従来どおり「最低1件」のみを見る。写真は追加（append）のみで既存分の削除・置換ができない仕様のため、編集のたびに規定枚数ちょうどを要求すると、他の項目を直すだけの編集や、規定枚数変更前に作成された既存データの編集が行えなくなってしまうため。
...
`section5_json` は `{ "photos": string[], "docs": { [field]: string | string[] } }` の形。`photos` は常に配列。`docs.*` は初回提出時は文字列1件、編集で追加された後は配列になる（フロント・API側とも文字列・配列の両方に対応済み）。`docs` のフィールドは `regulations`（団体規約）・`activityReport`（直近年度の活動報告書）・`financialReport`（直近年度の収支決算書）・`activityPlan`（直近年度の活動計画書）・`financialPlan`（直近年度の収支計画書、以上5点は必須）に加え、任意の `other`（その他の補足資料。機関誌、活動を紹介した新聞記事等、申請事業や日常活動のわかる補足資料、見積書・カタログなど）がある。
...
### 5. アップロードサイズ上限の確認（本番環境で特に重要）

要望申請フォームは、活動写真（3枚必須・1枚5MBまで）とPDF資料6種（うち必須5種＋
任意の「その他」1種、1点10MBまで）を同時に送信できる仕様のため、最大構成では
1リクエストが約75MBになり得る（`.user.ini` の `post_max_size`＝80Mに対してかなり
近い値のため、今後さらに添付項目を増やす場合は上限の見直しが必要になる）。

PHPの `post_max_size`（リクエスト全体の上限）がこれより小さいと、PHPは
リクエストを拒否するのではなく **`$_POST` / `$_FILES` を丸ごと空にしてしまう**。
その結果、実際には全項目入力済みであっても「団体名称は必須です」のように
ほぼ全項目が必須エラーとして返ってくる（＝サーバー側で正しく受信できていない）。
`post_max_size` の一般的なデフォルト値（多くの環境で `8M`）はこの用途には
不足しているため、そのままでは写真を数枚添付しただけで発生し得る。

これに対応するため `api/.user.ini` で `upload_max_filesize` / `post_max_size`
を引き上げている（PHP-FPM・mod_phpいずれの構成でも読み込まれる）。ただし
以下の点に注意すること。

- ホスティング環境によっては、共有サーバー側のポリシーでアップロード上限が
  別途制限されている場合がある。その場合は `.user.ini` だけでは上限を
  引き上げられないため、ホスティング側の設定・プランを確認する必要がある
- `.user.ini` の反映には反映待ち時間が発生する場合がある
  （PHP-FPMの `user_ini.cache_ttl` 既定値: 300秒）。デプロイ直後は反映されて
  いない可能性がある
- `post_max_size` と `upload_max_filesize` はPHP起動時に確定する設定のため、
  PHPコード側（`ini_set()`）から変更することはできない

完了報告フォームの領収書アップロード（`ReceiptUploader`）には、以前は枚数の
上限が無く、大量にアップロードされると同様に `post_max_size` を超えてしまう
可能性があった。これに対応するため、フォーム側（`maxFiles` のデフォルト値）
で最大5枚に制限している。活動実施写真（最大2枚・1枚5MBまで＝10MB）と合わせて
1リクエストになるため、領収書1ファイルあたりの上限（5MB）を踏まえて
10枚（最大50MB）とし、写真分と合わせても60MB程度に収まるよう
`.user.ini` の `post_max_size`（80M）とのバランスを取っている。
...
- 送信エラー（`submitError`）は、「次へ」「戻る」「一時保存」のいずれかを押した時点でクリアする。これが無いと、1回送信に失敗した後は別のSTEPへ移動してもエラー文言が表示され続け、あたかも全項目でエラーが起きているかのように見えてしまう
...
| 5 | 添付資料（活動写真3枚必須・PDF必須5種・その他資料（任意）・確認チェック）。編集時（トークンアクセス時）は添付が任意になり、未添付の場合は既存ファイルを保持する |
...
| 3 | 添付資料（活動写真2枚必須・領収書最大10枚・確認チェック）。編集時（トークンアクセス時）は添付が任意になり、未添付の場合は既存ファイルを保持する |
